  ...
  TransactionDownloader.prototype = {
    ...
    downloadTransactions: function (pack_size) {
      if (!this.busy) {
        ...
        this.busy         = true; 
        this.getTransactions(0, pack_size||10);
      }
    },

    /**
     * Forcefully download all transactions. 
     **/
    downloadAllTransactions: function (pack_size) {
      this.busy = true; 
      this.getTransactions(0, pack_size||10, true);
    },
    ...
    getTransactions: function (start, count, download_all) {
      ...
      function recurse(iterator) {
        if (iterator.hasMore()) {
          self.processTransaction(iterator.next(), function (inserted) {
            if (inserted) {
              new_count += 1;
            }
            recurse(iterator);
          ...
          if (inserted > 0 || download_all) {
            $timeout(function () {
              self.getTransactions(lastIndex+1, count);
            });
          }
          else {
            self.busy = false;
  ...
  Renderer.prototype = {
    ...
    _renderTransaction: function (transaction, translator) {
      ...
      switch (transaction.type) {
        ...
        case 1: {
          switch (transaction.subtype) {
            case 0: return sprintf('%s %s <strong>sent a message</strong> to %s (%s) %s %s', 
                              label(transaction),
                              render(TYPE.ACCOUNT, transaction.senderRS), 
                              render(TYPE.ACCOUNT, transaction.recipientRS), 
                              render(TYPE.JSON,'details',JSON.stringify(transaction)),
