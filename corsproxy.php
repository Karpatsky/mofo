...
$enable_native   = true;
$valid_url_regex = '/.*/';
$allowed_ports   = array( 7876, 7874, 7886, 7884, 6886, 6884 );
...
} else if ( !in_array( parse_url($url)['port'], $allowed_ports ) ) {

  // Invalid port.
  $contents = 'ERROR: invalid port ' + parse_url($url)['port'];
  $status = array( 'http_code' => 'ERROR' );
...
if ( isset($_GET['mode']) && $_GET['mode'] == 'native' ) {
  ...
  # header("Access-Control-Expose-Headers: "+ join(",", getallheaders()));
